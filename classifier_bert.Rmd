...
# 그림 목록 제거
lof : false
# 그림 목록의 제목 설정, 기본값은 "List of Abbreviations"
lof-title : "Acronyms"
---
...
#knitr::opts_chunk$set(dpi=50, out.width="100%")
...
```{r wcloud_wcupdate, fig.align = 'center', fig.width=6, fig.height=6, fig.cap="날짜 기준 월드컵 뉴스"}
library(bitReport)
...
```{r wcloud_wcupsim, fig.align='center', fig.width=6, fig.height=6, fig.cap="유사도 기준 월드컵 뉴스"}
news_worldcup_sim %>% 
  create_wordcloud(remove_n = 20, min_freq = 2)
```
...
```{r rules_plot, message = FALSE, warning = FALSE, fig.align='center', out.width="100%", dpi=30}
library("arulesViz")
...
```{r plot_cluster, fig.align='center', out.width="100%", dpi=40}
k <- 6
...
```{r plot_cluster_list, out.width="100%", dpi=30}
k <- 6
...
```{r wcloud_news_wcupsim, fig.align='center', fig.height=6, fig.width=6, fig.cap="월드컵 기사의 제목"}
news_worldcup_sim %>% 
  filter(id %in% clusters[[1]]) %>% 
...
```{r plot_top_prob, fig.align='center', fig.cap="Top beta 단어의 시각화", out.width="100%", dpi=30}
top_prob %>% 
...
```{r plot_coefs_tf, fig.align='center', fig.cap="예측에 영향을 주는 모델의 계수들 with TF", out.width="100%", dpi=30}
coefs_tf %>%
  group_by(estimate > 0) %>%
...
```{r plot_roctf, fig.align='center', fig.cap="ROC 커브", out.width="100%", dpi=30}
plot(roc_tf)
```
